...
import pyautogui, random, configparser
...
def fight_stage(status):
    ...
    #                             1:int(config['DEFALT_ABILITIES']['1']),
    #                             2:int(config['DEFALT_ABILITIES']['2']),
    #                             3:int(config['DEFALT_ABILITIES']['3']),
    #                             }
    ...
    while fight_started:
        ...
        while not isBoardReadyToFight(boardMinions):
            ...
            for i in range(my_minion_size):
                ...
                rightclick(minion_full_position)
                click(minion_full_position)

                default_target_skill = skill_positions[1]
                overloaded_target_skill = default_target_skill

                CardID = boardMinions[i]['CardID']
                overloaded_target_position = -1

                try:
                # if CardID in config:
                    print('Mionion %s selected skill number: %s' % (boardMinions[i]['entityName'], config['DEFALT_ABILITIES'][CardID]))
                    overload = config['DEFALT_ABILITIES'][CardID].split(",")
                    overloaded_target_skill = skill_positions[int(overload[0])]
        
                    overloaded_target_position = int(overload[1]) - 1
                except:
                    pass
                ...
                if overloaded_target_position >= 0:
                    overload_target_x = minion_pos_x[my_minion_size][overloaded_target_position]
                    overload_target_y = my_minion_y
                    overload_pos = [overload_target_x, overload_target_y]
                    click(overload_pos)
        ...
        if not fight_started: 
            ...
            return
        expected_wait_time = min(60, wait_exp_time - (time.time() - fight_timer_start))
        print('exp wait time required [%d] seconds' % (expected_wait_time))
